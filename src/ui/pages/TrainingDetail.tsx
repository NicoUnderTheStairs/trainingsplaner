...
const formatDate = (date: any): string => {
  ...
  const dayMonth = d.toLocaleDateString("de-CH", { day: "2-digit", month: "2-digit", year: "numeric" });
  return `${weekday}, ${dayMonth}`;
};

// ─── Duration bar ─────────────────────────────────────────────────────────────

const DurationBar = ({ planned, total }: { planned: number; total: number }) => {
  ...
  return (
    <div className="trainingdetail__durationbar">
      ...
      <span className={`trainingdetail__durationbar__label${over ? " trainingdetail__durationbar__label--over" : ""}`}>
        {planned} / {total} min planned
        {over && <span className="trainingdetail__durationbar__over"> (+{planned - total} over)</span>}
      </span>
    </div>
  );
};
...
const ShareDialog = ({ training, currentUserId, senderName, onClose }: ShareDialogProps) => {
  ...
  const handleShare = async () => {
    ...
    try {
      // 1. Write a copy of the training to each recipient's subcollection
      const recipientIds = Array.from(selected);

      await Promise.all(
        recipientIds.map((recipientId) =>
          setDoc(
            doc(collection(db, "users", recipientId, "trainings")),
            {
      ...
      // 2. Send a notification to each recipient
      try {
        await Promise.all(
          recipientIds.map((recipientId) =>
        ...
        // Non-critical — log but don't block the success state
    ...
    <div className="dialog__overlay" onClick={onClose}>
      <div className="dialog dialog--share" onClick={(e) => e.stopPropagation()}>
        ...
        <p className="dialog__body">
          Select teammates to share <strong>{training.title}</strong> with.
          It will appear in their training list with you as the author.
        </p>
        ...
        <div className="dialog__actions">
          ...
          >
            {shared ? (
              ...
              <>Share with {selected.size > 0 ? `${selected.size} ` : ""}{selected.size === 1 ? "person" : "people"}</>
            )}
          </button>
        </div>
      </div>
...
const TrainingDetail = () => {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      const snap = await getDoc(doc(db, "users", currentUserId, "trainings", trainingId));
  ...
  // ── Resolve sender name for share dialog ────────────────────────────────────
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        ...
        if (snap.exists()) {
          setSenderName((snap.data() as any).userName ?? currentUser?.email?.split("@")[0] ?? "Someone");
        }
      } catch {
  ...
  const handleEditStart = () => {
    if (!training) return;
    setEditData({ title: training.title, description: training.description, duration: training.duration, tags: training.tags ?? [] });
    setEditing(true);
  };
  const handleEditCancel = () => { setEditing(false); setEditData({}); };
  const handleEditChange = (field: keyof Training, value: unknown) => setEditData((prev) => ({ ...prev, [field]: value }));
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setEditing(false);
      setEditData({});
    } catch (e) { console.error("Error updating training:", e); }
    finally { setSaving(false); }
  };

  // ── Edit exercises ─────────────────────────────────────────────────────────
  const handleEditExercisesStart  = () => { setEditExercises([...(training?.exercises ?? [])]); setEditingExercises(true); };
  const handleEditExercisesCancel = () => { setEditingExercises(false); setEditExercises([]); };
  const handleExerciseDurationChange = (idx: number, val: string) =>
    setEditExercises((prev) => prev.map((e, i) => i === idx ? { ...e, duration: Math.max(0, parseInt(val) || 0) } : e));
  const handleRemoveExercise = (idx: number) => setEditExercises((prev) => prev.filter((_, i) => i !== idx));
  const handleSaveExercises = async () => {
    if (!trainingId || !training || !currentUserId) return;
    setSaving(true);
    try {
      await updateDoc(doc(db, "users", currentUserId, "trainings", trainingId), { exercises: editExercises });
      setTraining((prev) => prev ? { ...prev, exercises: editExercises } : prev);
      setEditingExercises(false);
      setEditExercises([]);
    } catch (e) { console.error("Error updating exercises:", e); }
    finally { setSaving(false); }
  };

  // ── Drag reorder ───────────────────────────────────────────────────────────
  const handleDragStart = (idx: number) => { dragIndex.current = idx; setDraggingIdx(idx); };
  const handleDragEnter = (idx: number) => { dragOverIndex.current = idx; setDragOverIdx(idx); };
  const handleDragEnd   = () => {
  ...
  if (!training) return <><Navigation /><p style={{ padding: "4rem 11.2rem" }}>Training not found.</p></>;
            ...
            <div className="trainingdetail__header__body">
              <div className="trainingdetail__header__main">
                ...
                <div className="trainingdetail__header__text">
                  ...
                  {editing ? (
                    <>
                      <input className="trainingdetail__edit__input trainingdetail__edit__input--title" value={editData.title ?? ""} onChange={(e) => handleEditChange("title", e.target.value)} placeholder="Training title" />
                      <textarea className="trainingdetail__edit__input trainingdetail__edit__input--description" value={editData.description ?? ""} onChange={(e) => handleEditChange("description", e.target.value)} placeholder="Description" rows={3} />
                      <div className="trainingdetail__edit__duration">
                        <label>Total duration</label>
                        <div className="trainingdetail__edit__duration__row">
                          <input type="number" className="trainingdetail__edit__input trainingdetail__edit__input--duration" value={editData.duration ?? ""} onChange={(e) => handleEditChange("duration", parseInt(e.target.value) || 0)} placeholder="90" min={1} />
                          <span>min</span>
                        </div>
                      </div>
                    ...
                    <>
                      ...
                      {training.description && <p className="trainingdetail__description">{training.description}</p>}
                ...
                <div className="trainingdetail__header__actions">
                  {editing ? (
                    ...
                    <>
                      ...
                      <button className="trainingdetail__btn trainingdetail__btn--danger" onClick={() => setDeleteConfirm(true)}>Delete</button>
                    </>
                  )}
                </div>
              </div>
          ...
          <div className="trainingdetail__exercises">
            <div className="trainingdetail__exercises__header">
              <div className="trainingdetail__exercises__header__left">
                <h2>Exercises <span>({editingExercises ? editExercises.length : exercises.length})</span></h2>
                <DurationBar planned={editingExercises ? editPlanned : totalPlanned} total={training.duration} />
              </div>
              <div className="trainingdetail__exercises__header__actions">
                {editingExercises ? (
                  <>
                    ...
                    <button className="btn__primary" onClick={handleSaveExercises} disabled={saving}>{saving ? "Saving..." : "Save"}</button>
                  </>
                ) : (
                  ...
                  (ex: SelectedExercise, index: number) =>
                    editingExercises ? (
                      <div key={`${ex.exerciseId}-${index}`}
                        className={["trainingdetail__exercises__item", "trainingdetail__exercises__item--edit", draggingIdx === index ? "trainingdetail__exercises__item--dragging" : "", dragOverIdx === index && draggingIdx !== index ? "trainingdetail__exercises__item--dragover" : ""].filter(Boolean).join(" ")}
                        draggable
                      ...
                      >
                        ...
                        <div className="trainingdetail__exercises__item__duration">
                          <input type="number" min={0} value={ex.duration || ""} placeholder="0" onChange={(e) => handleExerciseDurationChange(index, e.target.value)} onClick={(e) => e.stopPropagation()} />
                          <span>min</span>
                        </div>
                        <button className="trainingdetail__exercises__item__remove" onClick={() => handleRemoveExercise(index)}>×</button>
                      </div>
                    ) : (
                      <Link key={ex.exerciseId} to={`/exercise-detail/${ex.exerciseId}`} className="trainingdetail__exercises__item">
                        <span className="trainingdetail__exercises__item__index">{index + 1}</span>
                        <h3 className="trainingdetail__exercises__item__title">{ex.title}</h3>
      ...
      {deleteConfirm && (
        <div className="dialog__overlay" onClick={() => !deleting && setDeleteConfirm(false)}>
          <div className="dialog" onClick={(e) => e.stopPropagation()}>
            ...
            <p className="dialog__body"><strong>{training.title}</strong> will be permanently deleted. This cannot be undone.</p>
            <div className="dialog__actions">
