    ...
    <svg width="100%" height="100%" viewBox="0 0 560 440" fill="none" preserveAspectRatio="xMidYMid meet">
      ...
      {objects.map(([id, obj]: [string, any]) => (
        <g key={id} transform={`translate(${obj.x}, ${obj.y})`}>
          {obj.type === "pylon" && (
            <polygon points="0,-13 11,8 -11,8" fill="#FF8C00" stroke="#1E1E1E" strokeWidth={1.5} />
          )}
          {obj.type === "bench" && (
            <>
    ...
    const fetchExercises = async () => {
      try {
        ...
        const snapshot = userTeam
          ? await getDocs(query(collection(db, "Excercises"), where("team", "array-contains", userTeam)))
