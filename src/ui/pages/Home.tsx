...
export default function Home() {
  ...
  useEffect(() => {
    ...
    const fetch = async () => {
      try {
        ...
        const q = userTeam
          ? query(collection(db, "Excercises"), where("team", "array-contains", userTeam), orderBy("createdAt", "desc"), limit(4))
          : query(collection(db, "Excercises"), orderBy("createdAt", "desc"), limit(4));
        ...
        if (groupIds.length > 0) {
          ...
          await Promise.all(chunks.map(async (chunk) => {
            const vq = query(collection(db, "ExerciseVariants"), where("__name__", "in", chunk));
        ...
        const userTeam = userData?.team ?? "";
        const q = userTeam
          ? query(collection(db, "Excercises"), where("team", "array-contains", userTeam), limit(4))
  ...
  useEffect(() => {
    ...
    if (!userId) { setLoadingTr(false); return; }
    const fetch = async () => {
      try {
        const q = query(collection(db, "users", userId, "trainings"), where("sharedBy", "==", null), orderBy("createdAt", "desc"), limit(3));
        const snap = await getDocs(q);
        setTrainings(snap.docs.map((d) => ({ id: d.id, ...d.data() }) as Training));
      } catch {
        const snap = await getDocs(query(collection(db, "users", userId, "trainings"), limit(20)));
        setTrainings(
          snap.docs.map((d) => ({ id: d.id, ...d.data() }) as Training & { sharedBy?: string })
            .filter((t) => !t.sharedBy).slice(0, 3)
        );
  ...
  useEffect(() => {
    const userId = getAuth().currentUser?.uid;
    if (!userId) { setLoadingSh(false); return; }
    const fetch = async () => {
      try {
        const q = query(collection(db, "users", userId, "trainings"), where("sharedBy", "!=", null), orderBy("sharedBy"), orderBy("sharedAt", "desc"), limit(4));
        const snap = await getDocs(q);
        const raw = snap.docs.map((d) => ({ id: d.id, ...d.data() }) as SharedTraining);
        const senderIds = [...new Set(raw.map((t) => t.sharedBy).filter(Boolean))];
        const senderMap: Record<string, string> = {};
        await Promise.all(senderIds.map(async (uid) => {
          try {
            const { getDoc, doc } = await import("firebase/firestore");
            const senderDoc = await getDoc(doc(db, "users", uid));
            if (senderDoc.exists()) senderMap[uid] = (senderDoc.data() as any).userName ?? uid;
          } catch { senderMap[uid] = uid; }
        }));
        setSharedTrainings(raw.map((t) => ({ ...t, sharedByName: senderMap[t.sharedBy] ?? t.sharedBy })));
      } catch {
        try {
          const snap = await getDocs(query(collection(db, "users", getAuth().currentUser!.uid, "trainings"), limit(30)));
          setSharedTrainings(snap.docs.map((d) => ({ id: d.id, ...d.data() }) as SharedTraining).filter((t) => !!t.sharedBy).slice(0, 4));
        } catch { setSharedTrainings([]); }
      } finally { setLoadingSh(false); }
    };
    fetch();
  }, []);
      ...
      <div className="home">
        ...
        <div className="home__hero">
          <div className="home__hero__text">
            ...
            <h1 className="home__hero__name">{firstName}<span className="home__hero__dot">.</span></h1>
        ...
        <div className="home__grid">
          ...
          <section className="home__widget home__widget--trainings">
            <div className="home__widget__header">
              ...
              <button className="home__widget__link" onClick={() => navigate("/training-overview")}>View all →</button>
            </div>
            {loadingTr ? <Skeleton /> : trainings.length === 0 ? (
              <div className="home__widget__empty">
                <p>No trainings yet.</p>
                <button className="home__widget__cta" onClick={() => navigate("/create-training")}>+ Plan a training</button>
              </div>
            ) : (
              <div className="home__trainings__list">
                {trainings.map((tr) => (
                  <div key={tr.id} className="home__training__row" onClick={() => navigate(`/training-detail/${tr.id}`)}>
                    <div className="home__training__row__accent" />
                    <div className="home__training__row__info">
                      <span className="home__training__row__title">{tr.title}</span>
                      <span className="home__training__row__meta">{formatDate(tr.date)} · {tr.duration} min</span>
                    </div>
                ...
                <button className="home__trainings__new" onClick={() => navigate("/create-training")}>+ New training</button>
              </div>
            )}
          </section>
          ...
          <section className="home__widget home__widget--exercises">
            <div className="home__widget__header">
              ...
              <button className="home__widget__link" onClick={() => navigate("/exercise-overview")}>View all →</button>
            </div>
            {loadingEx ? <Skeleton /> : exercises.length === 0 ? (
              <div className="home__widget__empty">
                <p>No exercises yet.</p>
                <button className="home__widget__cta" onClick={() => navigate("/create-exercise")}>+ Add your first exercise</button>
              </div>
            ) : (
              <div className="home__exercises__grid">
                {exercises.map((ex) => {
                  ...
                  return (
                    <div key={ex.id} className="home__exercise__card" onClick={() => navigate(`/exercise-detail/${ex.id}`)}>
                      {/* Variant badge */}
                      {vc > 0 && (
                        <div className="home__exercise__card__variants" title={`${vc} variant${vc !== 1 ? "s" : ""}`}>
                          {vc}
                        </div>
                      )}
                      <div className="home__exercise__card__sketch">
                        {ex.sketch ? <SketchThumbnail sketch={ex.sketch} /> : <div className="home__exercise__card__sketch__empty" />}
                      </div>
          ...
          <section className="home__widget home__widget--shared">
            ...
            <p className="home__widget__desc">Trainings your teammates have shared with you.</p>
            {loadingSh ? <Skeleton /> : sharedTrainings.length === 0 ? (
              <div className="home__widget__empty">
                <p>No shared trainings yet.</p>
                <p className="home__widget__empty__hint">When a teammate shares a training with you it will appear here.</p>
              </div>
            ) : (
              <div className="home__shared__list">
                {sharedTrainings.map((tr) => (
                  <div key={tr.id} className="home__shared__card" onClick={() => navigate(`/training-detail/${tr.id}`)}>
                    <div className="home__shared__card__top">
                      <div className="home__shared__card__author">
                        <div className="home__shared__card__avatar home__shared__card__avatar--initials">
                          {(tr.sharedByName ?? tr.sharedBy)?.slice(0, 2).toUpperCase() ?? "?"}
                        </div>
                        <span className="home__shared__card__author__name">{tr.sharedByName ?? tr.author ?? tr.sharedBy}</span>
                      </div>
                      <span className="home__shared__card__date">{formatDate(tr.sharedAt ?? tr.date)}</span>
                    </div>
                    <h4 className="home__shared__card__title">{tr.title}</h4>
                    <div className="home__shared__card__meta">
                      <span>{tr.duration} min</span><span>·</span><span>{(tr.exercises ?? []).length} exercises</span>
                    </div>
