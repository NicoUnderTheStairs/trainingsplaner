...
const SketchEditor = ({ sketch, onChange }: { sketch?: SketchData; onChange: (s: SketchData) => void }) => {
  ...
  useEffect(() => { onChange(toSketchData(players, arrows)); }, [players, arrows]);
  ...
  const handlePlayerMouseDown = useCallback((e: React.MouseEvent, id: string) => {
    ...
    if (!p) return;
    dragRef.current = { playerId: id, offsetX: pt.x - p.x, offsetY: pt.y - p.y };
  ...
  const handleSVGMouseMove = useCallback((e: React.MouseEvent) => {
    ...
    if (dragRef.current) {
      ...
      setPlayers((prev) => prev.map((p) => p.id === playerId
        ? { ...p, x: Math.max(10, Math.min(SVG_W - 10, pt.x - offsetX)), y: Math.max(10, Math.min(SVG_H - 10, pt.y - offsetY)) }
        : p));
    }
  ...
  const handleDrop = useCallback((e: React.DragEvent) => {
    ...
    setPlayers((prev) => [...prev, { id: uid(), x: Math.max(10, Math.min(SVG_W - 10, pt.x)), y: Math.max(10, Math.min(SVG_H - 10, pt.y)), type, label: PLAYER_LABELS[type] }]);
  }, [getSVGPoint]);
  ...
  return (
    <div className="sketch__editor">
      <div className="sketch__toolbar">
        <div className="sketch__toolbar__group">
          ...
          {(Object.keys(PLAYER_COLORS) as PlayerType[]).map((type) => (
            <div key={type} className="sketch__palette__player" draggable title={type} onDragStart={(e) => e.dataTransfer.setData("playerType", type)} style={{ background: PLAYER_COLORS[type] }}>{PLAYER_LABELS[type]}</div>
          ))}
        </div>
        <div className="sketch__toolbar__group">
        ...
        <div className="sketch__toolbar__group sketch__toolbar__group--right">
          {selectedId && <button className="sketch__tool__btn sketch__tool__btn--danger" onClick={handleDelete}>× Delete</button>}
          <button className="sketch__tool__btn" onClick={() => { setPlayers([]); setArrows([]); setSelectedId(null); }}>✕ Clear all</button>
        </div>
      </div>
      <svg ref={svgRef} width={SVG_W} height={SVG_H} viewBox={`0 0 ${SVG_W} ${SVG_H}`} fill="none"
        ...
        onMouseLeave={handleSVGMouseUp} onDrop={handleDrop} onDragOver={(e) => e.preventDefault()}
      >
        <defs>
        ...
        {draftArrow && <line x1={draftArrow.x1} y1={draftArrow.y1} x2={draftArrow.x2} y2={draftArrow.y2} stroke="#aaa" strokeWidth={2} strokeDasharray={arrowStyle === "dashed" ? "6 4" : undefined} markerEnd="url(#ed-arrow-draft)" pointerEvents="none" />}
        {players.map((p) => (
          <g key={p.id} transform={`translate(${p.x}, ${p.y})`} onMouseDown={(e) => handlePlayerMouseDown(e, p.id)} onClick={(e) => { e.stopPropagation(); if (tool === "select") setSelectedId(p.id); }} style={{ cursor: tool === "select" ? "grab" : "default" }}>
      ...
      <p className="sketch__hint">{tool === "select" ? "Drag players onto the court · Click to select · Del to remove" : "Click and drag to draw an arrow"}</p>
    </div>
  );
};
...
const ExerciseDetail = () => {
  ...
  const handleSaveSketch = async () => {
    ...
    setSaving(true);
    try {
      await updateDoc(doc(db, "Excercises", exerciseId), { sketch: editSketch });
  ...
  const handleDelete = async () => {
    ...
    setDeleting(true);
    try {
      // 1. Clean up the variant group before removing the exercise doc
              ...
              {(hasVariants || loadingVariants) && (
                <div className="exercisedetail__variants">
                  <h3 className="exercisedetail__variants__title">
                    Variants
                    <span className="exercisedetail__variants__count">{variants.length}</span>
                  </h3>
